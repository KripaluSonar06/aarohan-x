...
import { Activity, AlertTriangle, Check, ChevronRight, CircleDollarSign, FileCheck2, Gauge, LayoutDashboard, Menu, Play, Search, ShieldCheck, Target, WalletCards, Zap } from 'lucide-react'
...
const fallback: Case[] = [
  { id: 'EVT_0001', customer: 'Customer 1', merchant: 'Spotify', amount: 199, reason: 'Insufficient funds', probability: .61, attempts: 1, status: 'Recovered', date: 'Today', communications: [], expectedGross: 0, channelCost: 0, netExpected: 0, ledger: [] },
  { id: 'EVT_0002', customer: 'Customer 2', merchant: 'Netflix India', amount: 4999, reason: 'Insufficient funds', probability: .58, attempts: 1, status: 'Recovered', date: 'Today', communications: [], expectedGross: 0, channelCost: 0, netExpected: 0, ledger: [] },
  { id: 'EVT_0003', customer: 'Customer 3', merchant: 'GymFlex', amount: 4999, reason: 'Insufficient funds', probability: .62, attempts: 4, status: 'In review', date: 'Today', communications: [], expectedGross: 0, channelCost: 0, netExpected: 0, ledger: [] },
]
...
const mapCase = (item: BackendCase): Case => ({
  ...
  explanation: item.decision_explanation, ledger: item.ledger || [],
})
...
export default function App() {
  ...
  useEffect(() => { refresh().catch(() => notify('Live data is temporarily unavailable. Showing the last available view.')) }, [])
  ...
  return <div className="app">
    ...
    <main><header><button className="mobile" onClick={() => setOpen(!open)}><Menu/></button><div className="crumb">⌘ Aarohan-X <ChevronRight size={14}/><b>{page}</b></div><div className="top-right"><span className="live"><i/> Workspace live</span><strong>KS</strong></div></header>
      <section className="content">{page === 'Command center' && <Command cases={cases} metrics={metrics} analytics={analytics} onBatch={batch}/>} {page === 'Cases' && <CasesPage cases={cases} query={query} setQuery={setQuery} onVerify={verify}/>} {page === 'Exceptions' && <Exceptions cases={cases} onVerify={verify}/>} {page === 'Experiments' && <Experiments results={experimentResults} onRun={async () => { try { setExperimentResults(await runExperiments()); notify('Strategy experiment completed.')} catch { notify('Experiment could not be completed. Please try again.') } }}/>} {page === 'Policy center' && <Policy notify={notify}/>}</section>
...
function RecoveryCopilot({ cases, metrics }: { cases: Case[]; metrics: Metrics | null }) {
  ...
  return <div className={`copilot ${open ? 'open' : ''}`}><button className="copilot-toggle" onClick={() => setOpen(!open)}>✦ Recovery Copilot</button>{open && <div className="copilot-panel"><b>Recovery Copilot</b><div className="copilot-messages">{messages.map((message, index) => <div className={message.from} key={index}>{message.text}</div>)}</div><div className="copilot-input"><input value={input} onChange={event => setInput(event.target.value)} onKeyDown={event => event.key === 'Enter' && send()} placeholder="Ask about this batch..."/><button onClick={send}>Send</button></div></div>}</div>
}
...
function CasesPage({ cases, query, setQuery, onVerify }: { cases: Case[]; query: string; setQuery: (value: string) => void; onVerify: (id: string) => void }) {
  ...
  return <><Title eyebrow="CASE MANAGEMENT" title="Recovery cases" desc="Monitor failed payments, customer outreach, and recovery outcomes."/><section className="card table-card"><div className="table-tools"><div className="search"><Search size={16}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search cases..."/></div></div><div className="table-wrap"><table><thead><tr><th>Case</th><th>Customer / merchant</th><th>Amount</th><th>Diagnosis</th><th>Contact activity</th><th>Status</th><th/></tr></thead><tbody>{filtered.map(item => <><tr key={item.id}><td><code>{item.id}</code><small>{item.date}</small></td><td><b>{item.customer}</b><small>{item.merchant}</small></td><td><b>{money(item.amount)}</b></td><td>{item.reason}</td><td>{item.communications.length ? `${item.communications.length} contact${item.communications.length === 1 ? '' : 's'}` : 'No contact'}</td><td><span className={`status ${item.status.toLowerCase().replace(' ', '-')}`}><i/>{item.status}</span></td><td>{item.status === 'In review' && <button className="table-verify" onClick={() => onVerify(item.id)}>Verify</button>}</td></tr>{(item.communications || []).map((communication, index) => <tr className="communication-row" key={`${item.id}-${communication.timestamp}-${index}`}><td/><td colSpan={6}><b>{communication.channel === 'text' ? 'Text nudge sent' : 'Voice call made'}</b><small>{communication.timestamp ? new Date(communication.timestamp).toLocaleString() : ''}</small>{communication.message && <p>{communication.message}</p>}{communication.transcript && <p>Call transcript: {communication.transcript}</p>}</td></tr>)}</>)}</tbody></table></div></section></>
}
